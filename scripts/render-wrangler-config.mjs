...
//
// This is the ONLY thing that produces wrangler.jsonc, and it runs before dev,
// test and deploy alike (see the package.json scripts). Terraform used to own
// this for deploys while this script covered local dev; now there is one code
// path, so local config cannot drift from deployed config.
//
...
// Custom domains are per-environment, so the routes block is only emitted when
// a DOMAIN is supplied. Local dev and the vitest suite render `[]`, which keeps
// `wrangler dev` off the zone API entirely.
//
...
const vars = {
  ...
  mutating_rate_limit_threshold: positiveInt("MUTATING_RATE_LIMIT_THRESHOLD", 60),
  read_rate_limit_threshold: positiveInt("READ_RATE_LIMIT_THRESHOLD", 600),
};
...
const withoutComments = rendered
  .replaceAll(/\/\*[\s\S]*?\*\//g, "")
  .replaceAll(/^\s*\/\/.*$/gm, "");
try {
...
console.log(`Rendered wrangler.jsonc (worker=${vars.worker_name}, routes=${domain || "none"})`);
