...
3. If you touch `wrangler.jsonc.tmpl` or the config renderer: say so in the PR. CI's `config-renders` job dry-runs an operator-shaped deploy, but a reviewer should know config changed
...
- **Lint**: `oxlint` with `--type-aware --deny-warnings`. Config at
  `.oxlintrc.json`. Type-aware linting needs the `oxlint-tsgolint` binary (a
  devDependency) and TypeScript 7. `no-floating-promises` is on and matters
  here — the Durable Object fires webhooks with retry loops, where a dropped
  promise fails silently. Don't disable rules inline without a comment on why.
...
- **Types**: `tsc --noEmit` on TypeScript 7 (the native-speed compiler, now
  shipped as stable `typescript` — the old `tsgo` / `@typescript/native-preview`
  pairing is gone). `strict: true` + `noUncheckedIndexedAccess` are on by
  default; don't weaken them. Avoid `any` unless you've documented a concrete
  reason in a single-line comment next to the cast.

  Note: `typescript@7` ships only a `tsc` binary — there is no `tsserver` until
  7.1. If your editor is pinned to the workspace TypeScript
...
[`docs/CICD.md`](docs/CICD.md#what-a-deploy-needs)).
...
secrets. Nothing committed upstream should need patching downstream. If
you're tempted to commit something operator-specific to a fork, file an
upstream issue first — the right shape is usually a new environment variable
upstream so every operator benefits.
...
- **Major** (`v2.0.0`, `v3.0.0`) — breaking changes to the
  operator-facing config surface: GitHub Actions
