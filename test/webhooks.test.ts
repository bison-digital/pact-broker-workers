...
// Outbound webhook delivery is intercepted by the `outboundService` handler in
// vitest.config.ts, which answers https://webhook.example/hook with 200 and
...
// beforeEach because each request consumed an interceptor; outboundService is
// configured once and needs no per-test setup.
//
// Mocking globalThis.fetch — the migration guide's suggestion — would not work
// here: the request is issued from inside PactBrokerDO.fireWebhook(), and the
// "global mocks apply to the main worker" guarantee does not extend into a
// Durable Object.
