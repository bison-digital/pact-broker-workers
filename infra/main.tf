...
# Rate limiting is enforced inside the Worker via the Workers Rate
# Limiting API; the bindings are declared in wrangler.jsonc.tmpl
# (`ratelimits` block) and consumed by src/middleware/rate-limit.ts.
# Thresholds flow through `var.mutating_rate_limit_threshold` and
# `var.read_rate_limit_threshold` (see wrangler.tf templatefile call).
# No zone-level `cloudflare_ruleset` resource is needed; the Worker
# binding works on every Cloudflare plan including Free and requires
# no `Zone WAF: Edit` permission on the deploy API token.
