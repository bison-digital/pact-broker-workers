...
app.get("/provider/:provider/consumer/:consumer/pact-version/:sha", async (c) => {
  // Validate URL parameters
  const providerResult = validateParam(c, nameSchema, c.req.param("provider"), "provider");
  if (!providerResult.valid) return providerResult.response;
  const providerName = providerResult.value;
  ...
  const broker = getBroker(c.env);
  const result = await broker.getPactByContentShaFull(providerName, consumerName, sha);
...
app.get("/provider/:provider/latest", async (c) => {
  ...
  const response = {
    ...
    _embedded: {
      pacts: pacts.map((p) => buildPactResponse(hal, p.pact, p.consumer, p.provider, p.version)),
    },
  };
...
app.get("/latest", async (c) => {
  ...
  for (const p of pacticipants) {
    ...
    for (const pact of pacts) {
      allPacts.push(buildPactResponse(hal, pact.pact, pact.consumer, pact.provider, pact.version));
    }
  }
